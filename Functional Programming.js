...
//6 - refactor-global-variables-out-of-functions
  ...
  function add(p_array, bookName) {
    let auxArray = JSON.parse(JSON.stringify(p_array));
    auxArray.push(bookName);
    return auxArray;
  ...
  // Change code below this line
  function remove(p_array,bookName) {
    let auxArray = JSON.parse(JSON.stringify(p_array));
    const book_index = auxArray.indexOf(bookName);
    if (book_index >= 0) {

      auxArray.splice(book_index, 1);
      return auxArray;
...
//9- use-the-filter-method-to-extract-data-from-an-array
  ...
  function sliceArray(anim, beginSlice, endSlice) {
    // Only change code below this line
    const auxArray = anim.slice(beginSlice, endSlice);
    return auxArray;
    // Only change code above this line
  }
...
//12- Remove Elements from an Array Using slice Instead of splice
  ...
  function nonMutatingSplice(cities) {
    // Only change code below this line
  const auxArray = cities.slice(0, 3);
  return auxArray;
...
//13- Combine Two Arrays Using the concat Method
  ...
  function nonMutatingConcat(original, attach) {
    // Only change code below this line
    const auxArray = original.concat(attach);
    return auxArray;
...
//15- Use the reduce Method to Analyze Data
  ...
  const watchList = [
    ...
    function getRating(watchList) {
      // Only change code below this line
      let averageRating = watchList.filter(peliF => peliF.Director === "Christopher Nolan")
      .map(peliM => parseFloat(peliM.imdbRating))
      .reduce((sum, immdbRating) => sum + immdbRating) / 
...
//16- Use Higher-Order Functions map, filter, or reduce to Solve a Complex Problem
  ...
  const squareList = arr => {
    // Only change code below this line
    return arr.filter(ele => Number.isInteger(ele) && ele > 0)
...
//18- Return a Sorted Array Without Changing the Original Array
  ...
  function nonMutatingSort(arr) {
    // Only change code below this line
    const auxArray = [];
    return auxArray.concat(arr).sort(function(a,b){
      return a - b; 
    }); 
